...
describe(TEST_NAME, function() {
  ...
  describe("#expire()", function() {
    before(function(done) {
      async.series([
        function(next) {
          redis.set("to_expire1", "to_expire value1", next);
        },
        function(next) {
          redis.push("to_expire_q1", "to_expire_q value1", next);
        }
      ], done);
    });
    ...
    it("should set the ttl of existing list key", function(done) {
      redis.expire("to_expire_q1", 1, function(err, result) {
        expect(err).to.not.exist;
        expect(result).to.equal(1);
        setTimeout(function() {
          redis.get("to_expire_q1", function(err, value) {
            expect(err).to.not.exist;
            expect(value).to.not.exist;
            done();
          });
